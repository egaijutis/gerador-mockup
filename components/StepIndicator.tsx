...
export const StepIndicator: React.FC<StepIndicatorProps> = ({ currentStep }) => {
  const steps = [
    ...
    { id: AppStep.DESCRIBE, label: 'Aplicação' },
  ...
  return (
    <div className="w-full max-w-4xl mx-auto mb-8 px-4">
      <div className="flex items-center justify-between relative">
        <div className="absolute left-0 top-1/2 transform -translate-y-1/2 w-full h-1 bg-zinc-800 -z-10 rounded"></div>
        ...
        {steps.map((step, index) => {
          ...
          let circleClass = "bg-zinc-800 border-2 border-zinc-600 text-zinc-400";
          ...
          return (
            <div key={step.id} className="flex flex-col items-center gap-2 bg-zinc-950 px-2">
              ...
              <span className={`text-xs sm:text-sm font-medium ${isActive || isCompleted ? 'text-white' : 'text-zinc-500'}`}>
                {step.label}
              </span>
            </div>
          );
